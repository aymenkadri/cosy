...
public class AuteurAccueilView extends Composite implements Editor<Person> {
	...
	interface CellTablesUiBinder extends UiBinder<Widget, AuteurAccueilView> {
	}

	interface Driver extends SimpleBeanEditorDriver<Person, AuteurAccueilView> {
	}

	Driver driver = GWT.create(Driver.class);

	public AuteurAccueilView() {
		...
		favorite.setAcceptableValues(Arrays.asList(Person.Favorite.values()));
	...
	public void onClickAdd5Entity(ClickEvent e) {
		...
		for (int i = 0; i < 5; i++) {
			...
			p.setFavorite(Person.Favorite.values()[Random.nextInt(Person.Favorite.values().length)]);
